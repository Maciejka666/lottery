...
export function Lottery() {
  ...
  const formatAddress = (address: string) => {
    if (!address) return 'N/A'; // Zwróć 'N/A', jeśli adres jest pusty
    return `${address.slice(0, 6)}...${address.slice(-4)}`; // Pokazuje początkowe 6 i ostatnie 4 znaki
  };
        ...
        {!isConnected ? (
          ...
          <>
            <p className="text-gray-700 text-center mb-4">
              Lottery ends in: <span className="font-bold text-green-600">{timeLeft > 0 ? formatTime(timeLeft) : 'Lotteria zakończona'}</span>
            </p>
            <p className="text-gray-700 text-center mb-4">Contract balance: <span className="font-bold text-green-600">{!readIsPending && formatEther(BigInt(balance))}ETH</span>
            </p>
            <p className="text-gray-700 text-center mb-4">
              Tickets Owned: <span className="font-bold text-green-600">{!readIsPending && ownedTickets}</span>
            </p>
            <p className="text-gray-700 text-center mb-4">
              Last Winner: <span className="font-bold text-blue-600">{formatAddress(lastWinner)}</span>
            </p>
            <p className="text-gray-700 text-center mb-4">
              Last Prize: <span className="font-bold text-blue-600">{formatEther(BigInt(lastPrize)) || '0'} ETH</span>
            </p>
            <button
              onClick={buyTicket}
